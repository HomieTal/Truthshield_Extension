...
# --- AI Functions ---
def call_groq_api(messages, api_key, temperature=0.2, max_tokens=512):
    headers = {
...
def analyze_with_ai(news_text, api_key, is_followup=False, user_question=None):
    try:
...
def analyze():
    try:
        ...
        auto_ai_analysis = data.get('auto_ai_analysis', False)
        ...
        rf_result, rf_confidence, lr_result, lr_confidence = get_model_predictions(
            rf_model, lr_model, vectorizer, news_text
        )

        ai_result = None
        if auto_ai_analysis:
            ai_result = analyze_with_ai(news_text, api_key)

        # Determine final verdict
        confidence = (rf_confidence + lr_confidence) / 2
        verdict = lr_result if lr_result == rf_result else "Uncertain"
        if ai_result and "fake news" in ai_result.lower():
            verdict = "Fake News"
            confidence = max(confidence, 0.9)  # Boost confidence if AI agrees
        elif ai_result and "real news" in ai_result.lower():
            verdict = "Real News"
        ...
        return jsonify({
            ...
            'ai_result': ai_result,
            'analysis_details': ai_result or perform_rule_based_analysis(news_text)
        })
    except Exception as e:
...
def ask():
    try:
        ...
        if not news_text or not question:
            return jsonify({'error': 'Missing news text or question'}), 400
        if not api_key:
            return jsonify({'error': 'No API key provided'}), 401

        answer = analyze_with_ai(news_text, api_key, is_followup=True, user_question=question)
        return jsonify({'answer': answer})
    except Exception as e:
        print(f"❌ Error in /ask: {e}")
        return jsonify({'error': str(e)}), 500

def process_news(news_text, rf_model, lr_model, vectorizer, api_key):
    print(f"\n📰 Analyzing: {news_text}")
    start_time = time.time()
    ...
    if lr_pred == "Fake News" and rf_pred == "Real News":
        ...
        ai_check = analyze_with_ai(news_text, api_key)
        print(f"🔄 AI Analysis:\n{ai_check}")
        ...
        ask_followup_question(news_text, api_key)
        return
...
def ask_followup_question(news_text, api_key):
...
def test_groq_connection(api_key):
    ...
    response = call_groq_api(messages, api_key, temperature=0.1, max_tokens=50)
...
def main():
    ...
    parser.add_argument("--api-key", type=str, help="Grok API key for testing")
    ...
    if args.test_groq:
        if not api_key:
            print("❌ Please provide an API key with --api-key to test Groq connection")
            return
    ...
    if args.news:
        if not api_key:
            print("❌ Please provide an API key with --api-key to analyze news")
            return
        process_news(args.news, rf_model, lr_model, vectorizer, api_key)
    else:
        ...
        while True:
            ...
            if not api_key:
                print("⚠️ API key required for analysis.")
                continue
                
            process_news(news_text, *current_models, api_key)
